"""
    init_constraint!(com::CS.CoM,   constraint::CS.LinearConstraint,fct::SAF{T}, set::Union{MOI.LessThan{T}, CS.LessThan{T}};
                     active = true)
...
"""
    prune_constraint!(com::CS.CoM, constraint::LinearConstraint, fct::SAF{T}, set::Union{MOI.LessThan{T}, CS.LessThan{T}}; logs = true) where T <: Real
...
function prune_constraint!(
    ...
    # TODO: Make sure to recompute if a node in a different part of the tree
    # meaning: can work easily if in full DFS mode but otherwise one needs to be careful
...
"""
    still_feasible(com::CoM, constraint::LinearConstraint, fct::SAF{T}, set::Union{MOI.LessThan{T}, CS.LessThan{T}}, index::Int, val::Int) where T <: Real

function solve_str8ts(grid, white; backtrack=true, all_solutions=false, keep_logs=false, logging=[:Info, :Table])
    straight_tables = Vector{Array{Int,2}}(undef, 9)
    for i=1:9
        straight_tables[i] = get_straights(collect(1:9), i)
    end

    m = Model(optimizer_with_attributes(CS.Optimizer, "backtrack" => backtrack, "keep_logs"=>keep_logs, "all_solutions" => all_solutions, "logging" => logging))
